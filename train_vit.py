...
def train(config_path, train_hospital: int, data_dir):
    ...
    # torch.nn.init.constant_(model.heads.head.weight, 1e-4)
    # torch.nn.init.constant_(model.heads.head.weight, 0)
    ...
    save_model_from_epoch = config.config['DISTANCE_START_EPOCH'] + 1
    model_path = Path('ckpts') / 'vit_b_16' / f'{train_hospital}.pth'
    ...
    for epoch in range(config.config['TOTAL_EPOCHS']):
        ...
        if total_val_loss < best_loss:
            if not is_baseline_run(config.config) and epoch < save_model_from_epoch:
                ...
                model_save_dir = Path('ckpts') / 'vit_b_16'
...
def run_epoch(model, config, dataloader, epoch, optimizer, scheduler):
    ...
    for batch_idx, data in enumerate(dataloader, 0):
        ...
        x_out = model(x)
        x_embeddings = features['encoder'].flatten(start_dim=1)
        ...
        mask_embeddings = features['encoder'].flatten(start_dim=1)
